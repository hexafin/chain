	...
	return new Promise((resolve, reject) => {
		try {
			...
			if (notificationType == "wallet:addresses:new-payment") {
				...
				const amount =
					request.params.data.additional_data.amount.amount;
				const currency = request.params.additional_data.amount.currency;
				const tx = request.params.additional_data.hash;
				const coinbaseTransactionId =
					request.params.additional_data.transaction.id;
				const timestampCoinbaseCreated = Math.floor(
					Date.parse(request.params.created_at) / 1000
				);
				...
				firestore
					...
					.then(snapshot => {
						...
						const newTransaction = new Promise(
							(resolve, reject) => {
								coinbase.getExchangeRates(
									...
									(error, rates) => {
										...
										const exchangeRate =
											rates.data.rates[defaultCurrency];
										...
										firestore
											...
											.then(() => {
												slack(
													"chain:coinbase:createTransaction:success",
													email,
													amount
												);
											})
											.catch(error => {
												slack(
													"chain:coinbase:createTransaction:failure",
													email,
													error.toString()
												);
												reject(error);
						...
						const updateBalance = new Promise((resolve, reject) => {
							...
							firestore
								...
								.then(() => {
									slack(
										"chain:coinbase:updateBalance:success",
										email,
								...
								.catch(error => {
									slack(
										"chain:coinbase:updateBalance:failure",
										email,
										error.toString()
									);
									reject(error);
						...
						Promise.all([newTransaction, updateBalance])
							.then(() => {
								resolve(
									"transaction approved and balances updated"
								);
							})
							.catch(error => {
								reject(error);
					...
					.catch(error => {
						slack(
							"chain:coinbase:queryAddress:failure",
							error.toString()
						);
						reject(error);
					});
